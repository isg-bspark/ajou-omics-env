...
- **기준 패널 둘 — 조건(ctrl vs stim) 색 UMAP 과 celltype 색 UMAP — 이 같은 figure 안에
  함께** 있는가. 하나라도 빠지면 pathway 활성 영역이 조건 때문인지 세포 타입 때문인지
  대조할 기준이 없으므로 완결성을 깎는다. 두 기준 패널은 pathway 패널과 같은 임베딩·
  같은 축 범위여야 한다 — 파일을 따로 빼 놨다면 규격 위반이다
...
- **ctrl vs stim 을 구분한 stacked violin plot**(`results/07_functional/figures/pathway_stacked_violin.png`
  ...
  채운 것이므로 완결성을 깎는다
- 그 그림에서 **같은 celltype 의 ctrl/stim 을 나란히 비교할 수 있는가.** 조건이 둘
  (ctrl/stim)뿐인데 방법 B(`seaborn` split violin, 바이올린 하나가 좌우로 갈린다)를
  쓰지 않았다면 왜 방법 A 로 갔는지 근거가 있는지 본다. 방법 B 를 썼다면 **패널을 나눈
  기준이 pathway 인지** 본다 — celltype 별로 패널을 나눠 패널마다 바이올린 한 쌍만
  남았다면 규격 위반이다
- **바이올린이 납작한 선처럼 눌려 있지 않은가.** 가장 흔한 원인은 방향과 `figsize` 의
  불일치다. `sc.pl.stacked_violin` 은 기본 방향이 **그룹=행(y축)** 이라 그룹 수에 맞춰
  늘려야 하는 것이 높이인데, 너비만 그룹 수에 비례시키고 높이를 고정하면
  (`figsize=(0.55 * n_groups, 6)` 류) 12~18개 그룹이 눌려 전부 납작해진다. 코드에
  `swap_axes=True` + 너비 비례, 또는 기본 방향 + 높이 비례 중 하나가 있는지 확인한다.
  값이 아니라 그릴 자리의 문제이므로 정확성이 아니라 완결성을 깎는다
- **그룹별 세포 수를 확인할 수 있는가.** `density_norm="width"` 로 그렸다면 바이올린
  폭이 세포 수를 반영하지 않으므로, 그룹별 세포 수가 같은 단계의 표에 남아 있어야
  한다. 둘 다 없으면 세포 몇 개짜리 그룹의 분포를 2000개짜리와 같은 크기로 읽게 되므로
  완결성을 깎는다
